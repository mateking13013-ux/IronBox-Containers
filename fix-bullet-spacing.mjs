...
function generateCleanBulletList(product) {
  ...
  // Generate HTML list with no spacing
  const listItems = [
    `Size: ${displaySize} ${containerType}`,
    `Dimensions: ${specs.dims}`,
    `Volume: ${specs.volume} storage capacity`,
    `Condition: ${condition}`,
    `Features: ${features.slice(0, 3).join(', ')}`,
    `Certification: ISO/CSC approved`,
    `Max Load: ${specs.maxWeight} | Stackable | Weatherproof`
  ];

  // Create compact HTML
  const html = '<ul style="margin: 0; padding-left: 20px; line-height: 1.6;">\n' +
...
async function updateAllProducts() {
  console.log('🚀 Updating all products with clean bullet list format (no spacing)...\n');
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    // Generate clean bullet list HTML
    const bulletListHTML = generateCleanBulletList(product);
    ...
    try {
      const updateResponse = await fetch(`${WP_REST_URL}/wc/v3/products/${product.id}`, {
        ...
        body: JSON.stringify({
          short_description: bulletListHTML
        })
      });

      if (updateResponse.ok) {
        console.log(`✅ Updated with clean bullet list`);
        successCount++;
      } else {
